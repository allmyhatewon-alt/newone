...
async function main() {
        ...
        blocks: JSON.stringify([
          {
            ...
            type: "TEXT",
            order: 0,
            config: { heading: "welcome to my space", body: "this is your personal canvas. customize it like a landing page — text, music, links, embeds." },
          },
          {
            id: "links",
            type: "SOCIAL_LINKS",
  ...
  if (general) {
    const existingPost = await prisma.post.findFirst({
      where: { boardId: general.id, authorId: admin.id, title: "welcome to peng hub" },
    });
    if (!existingPost) {
      await prisma.post.create({
        data: {
          boardId: general.id,
          authorId: admin.id,
          title: "welcome to peng hub",
          body: "this is the place. clip. chat. customize. step through the portal to my Space.",
          flair: "ANNOUNCEMENT",
          isPinned: true,
        },
      });
